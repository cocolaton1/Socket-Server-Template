...
function handleMessage(ws, data, userID) {
    if (data instanceof Buffer) {
        // Handle binary data
        broadcastBinary(ws, data, userID);
    } else {
        try {
...
function broadcastBinary(senderWs, binaryData, userID) {
    usersInChat.forEach((user, id) => {
        if (user.ws.readyState === WebSocket.OPEN && user.wantsBinary && id !== userID) {
